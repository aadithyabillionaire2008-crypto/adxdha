...
router.post('/invoice', async (req, res) => {
  ...
  const invoice_no = `GPE-${new Date().getFullYear()}-${String(Date.now()).slice(-6)}`;
  ...
  const tx = db.transaction(() => {
    ...
    for (const item of items) {
      db.prepare('INSERT INTO invoice_items (id,invoice_id,product_id,description,quantity,price,tax_rate,line_total) VALUES (?,?,?,?,?,?,?,?)').run(nanoid(), invoice.id, item.product_id, item.description, item.quantity, item.price, item.tax_rate || 0, item.quantity * item.price);
